...
class TShirtBot:
    ...
    def _nuclear_check(self):
        ...
        # if DEBUG:
        #     print('[NUCLEAR]', '-' * 32)
        #     print('ENEMY to my center %s' % enemy_to_center_distance)
        #     print('MY UNIT dist %s, my unit VISION %s' % (my_unit_to_enemy_distance, init_vision))
        #     print('CELL %s, my_unit x:%s, y:%s' % (cell, self.my_vehicles[unit_id].x, self.my_vehicles[unit_id].y))
        ...
        if self.my_initial_vehicles[unit_id].aerial:
            ...
            if terrain == TerrainType.FOREST:
                vision = init_vision * 0.8
        # if DEBUG:
        #     print('RESULT VISION %s, distance to enemy %s' % (vision, my_unit_to_enemy_distance))

        # if enemy_to_center_distance < 50:
        #     # centers_dist = math.sqrt((self.enemy_center[0] - self.my_center[0]) ** 2 + (self.enemy_center[1] - self.my_center[1]) ** 2)
        #     # if DEBUG:
        #     #     print('Distance centers %s' % centers_dist)
        #     # my_unit_to_enemy_distance, unit_id = self._closest_my_vehicle_to_enemy_center()
        #     # if my_unit_to_enemy_distance < self.my_initial_vehicles[unit_id].vision_range * 0.8:
